...
<section class="content">
	<div class="box">
		...
		<div class="box-body">
			...
			<hr>
			<h3>{{ __('adminstaticword.Meeting') }} Type :@if($response['type'] == '2') Scheduled Meeting @elseif($response['type'] == '3') Recurring Meeting with no fixed time @else Recurring Meeting with fixed time @endif</h3>
			<hr>
			<h3>{{ __('adminstaticword.Meeting') }} Topic : {{ $response['topic'] }}</h3>
			<hr>
			<h3>{{ __('adminstaticword.Meeting') }} Agenda :{{ isset($response['agenda']) ? $response['agenda'] : "" }}</h3>
			<hr>
