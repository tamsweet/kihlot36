...
<section class="content">
	<div class="box">
		...
		<div class="box-header with-border">
			<div class="box-title">
			{{ __('adminstaticword.Edit') }} {{ __('adminstaticword.Meeting') }} : 
			</div>
		</div>
		...
		<div class="box-body">
			<form autocomplete="off" action="{{ route('googlemeet.update',$googlemeetedit['meeting_id']) }}" method="POST" enctype="multipart/form-data">
				    ...
				    <li class="tg-list-item">
				        <input class="tgl tgl-skewed" id="link_by" type="checkbox" name="link_by" {{ isset($googlemeetedit['link_by']) == 'course' ? 'checked' : '' }}>
				...
				<div class="form-group" style="{{ isset($googlemeetedit['link_by']) == 'course' ? '' : 'display:none' }}" id="sec1_one">
					...
					<select name="course_id" id="course_id" class="form-control js-example-basic-single">
	                    ...
	                    @foreach($course as $caat)
	                      ...
	                      <option {{ isset($googlemeetedit['course_id']) == $caat->id ? 'selected' : "" }} value="{{ $caat->id }}">{{ $caat->title }}</option>
	                    @endforeach 
	                </select>
				</div>
					...
					<div class="form-group">
						...
						<input type="text" name="topic" value="{{ $googlemeetedit['meeting_title'] }}" class="form-control" required>
					</div>
					...
					<div class="form-group" id="sec4_four">
                        ...
                        <div class='input-group date' id='datetimepicker1'>
                          <input name="start_date" type='text' value="{{ $googlemeetedit['start_time'] }}" class="form-control" />
					...
					<div class="form-group" id="sec5_four">
                        ...
                        <div class='input-group date' id='datetimepicker2'>
                          <input name="end_date" type='text' value="{{ $googlemeetedit['end_time'] }}" class="form-control" />
                          <span class="input-group-addon">
                              <span class="glyphicon glyphicon-calendar"></span>
                          </span>
                        </div>
					...
					<div class="form-group" id="sec3_three">
						...
						<input type="number" min="1" value="{{ $googlemeetedit['duration'] }}" name="duration" class="form-control">
					</div>
					...
					<div class="form-group">
						...
						<input type="text" name="description" value="{{ $googlemeetedit['agenda'] }}" class="form-control" required>
					</div>
					...
					<div class="m-1 form-group">
							<button class="btn btn-success btn-md">{{ __('adminstaticword.Update') }} {{ __('adminstaticword.Meeting') }}</button>
						</div>
